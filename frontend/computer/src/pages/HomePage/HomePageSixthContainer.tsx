...
export default function HomePageSixthContainer() {
  ...
  const [currentInd, setCurrentInd] = useState<number | null>(null);
  const paraRefs = useRef<(HTMLDivElement | null)[]>([]);
  const contentRefs = useRef<(HTMLDivElement | null)[]>([]);
  function handleExpand(ind: number) {
    setCurrentInd(ind);
    let isExpandCopyArr = [...isExpand];
    isExpandCopyArr[ind] = !isExpandCopyArr[ind];
    setIsExpand(isExpandCopyArr);
  }
  useEffect(() => {
    function handleResize() {
      if (currentInd === null) return;
      const paraHeight = paraRefs.current[currentInd]?.scrollHeight ?? 0;
      const contentHeight = contentRefs.current[currentInd]?.scrollHeight ?? 0;
      console.log(contentHeight, paraHeight);
    }
        ...
        <div className={styles.faqContainer}>
          {Arr &&
            Arr.map((item, ind) => {
              ...
              const maxHeight = isExpand[ind]
                ? contentHeight // expanded
                : contentHeight - paraHeight; // collapsed
              return (
                <div
                  key={ind}
                ...
                >
                  ...
                  >
                    <p>{item.title}</p>
                    <TiArrowSortedDown className={styles.faqContainerIcon} />
                  </div>
